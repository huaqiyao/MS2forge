...
def collate_with_spectrum_features(data_list):
    """collate_with_spectrum_features implementation."""
...
def collate_msg_diffms(data_list):
    """collate_msg_diffms implementation."""
    from torch_geometric.data import Batch
    ...
    # ---- MS2Forge align encoder dense X / E / y / node_mask ----
    ...
    for i, d in enumerate(data_list):
        ...
        triu = torch.triu_indices(n, n, offset=1)   # [2, n*(n-1)/2]

        edge_type_mat = torch.zeros(n, n, dtype=torch.long)
        if d.edge_index.size(1) > 0:
            ei = d.edge_index
            etype = d.edge_attr.argmax(dim=-1).long()
            edge_type_mat[ei[0], ei[1]] = etype

        halfedge_type_local = edge_type_mat[triu[0], triu[1]]
        halfedge_idx = triu + node_offset

        halfedge_index_chunks.append(halfedge_idx)
        halfedge_type_chunks.append(halfedge_type_local)
        halfedge_type_batch_chunks.append(
            torch.full((halfedge_type_local.size(0),), i, dtype=torch.long)
        )
        node_offset += n
...
def collate_smiles_for_graph2mol(data_list):
    """collate_smiles_for_graph2mol implementation."""
    from types import SimpleNamespace
...
def make_msg_diffms_collate_with_cache(zms_cache, zmol_target_cache=None):
    """make_msg_diffms_collate_with_cache implementation."""


    _smi_canon_cache = {}
    def _canon(smi):
        if smi in _smi_canon_cache:
            return _smi_canon_cache[smi]
        from rdkit import Chem
        try:
            mol = Chem.MolFromSmiles(smi)
            cano = Chem.MolToSmiles(mol, canonical=True, isomericSmiles=False) if mol else None
        except Exception:
            cano = None
        _smi_canon_cache[smi] = cano
        return cano

    def _collate(data_list):
        filtered = [d for d in data_list
                    if getattr(d, 'mol_id', None) in zms_cache]
        if zmol_target_cache is not None:
            filtered = [d for d in filtered
                        if _canon(getattr(d, 'smiles', '')) in zmol_target_cache]
        if not filtered:
            return None
        ...
        if zmol_target_cache is not None:
            zmol_list = [zmol_target_cache[_canon(smi)].float() for smi in batch.smiles]
            batch.zmol_target = torch.stack(zmol_list, dim=0)        # [B, 512]
        else:
            batch.zmol_target = None

        orig_to = batch.to
        def _to_with_cond(self, device):
            orig_to(device)
            self.cond_emb_cached = self.cond_emb_cached.to(device)
            if self.zmol_target is not None:
                self.zmol_target = self.zmol_target.to(device)
            return self
...
def make_smiles_collate_with_cache(zmol_cache):
    """make_smiles_collate_with_cache implementation."""
    _canon_cache_local = {}
    def _canon(smi):
        if smi in _canon_cache_local:
            return _canon_cache_local[smi]
        from rdkit import Chem
        try:
            mol = Chem.MolFromSmiles(smi)
            cano = Chem.MolToSmiles(mol, canonical=True, isomericSmiles=False) if mol else None
    ...
    def _collate(data_list):
        ...
        for d in data_list:
            ...
            if smi is None:
                continue
            cano = _canon(smi)
            if cano is None or cano not in zmol_cache:
                continue
